    ...
    <div className='py-28 md:py-35 md:px-16 lg:px-24 xl:px-32'>
      ...
      <div className='flex flex-col lg:flex-row mt-6 gap-6'>
        <div className='lg:w-1/2 w-full'>
            <img src={mainimg} alt="" className='w-full rounded-xl 
            shadow-lg object-cover' />
        </div>
        <div className='grid grid-cols-2 gap-4 lg:w-1/2 w-full'>
            {room?.images.length> 1 && room.images.map((image, index)=>(
                <img onClick={()=>setmainimg(image)}
                 key={index} src={image}
                 className={`w-full rounded-xl shadow-md object-cover cursor-pointer ${mainimg === image && 'outline-3 outline-orange-500'}`} />
            ))}
        </div>
      </div>
      {/* ROOM HIGHLIGHTS */}
      ...
      mx-auto mt-16 max-x-6xl '>
             <div className='flex flex-col flex-wrap md:flex-row items-start md:items-center gap-4 md:gap-10 text-gray-500'>
                  ...
                  <div className='flex flex-col'>
                    ...
                    className='max-w-20 rounded border border-gry-300 px-3 py-2 mt-1.5 outline-none' required />
                  </div>
             </div>
      ...
      <div className='flex flex-col items-start gap-4'>
           <div className='flex gap-4'> 
            <img src="https://cdn.vectorstock.com/i/1000v/82/50/elegant-u-letter-logo-beauty-women-vector-27938250.jpg" alt="" className='
            h-14 md:h-18 md:w-18 rounded-full' />
            <div>
